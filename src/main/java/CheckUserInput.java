    ...
    public void checkInput(SendMessage sendMessage, String userInputText) {
        if (!userInputText.isEmpty()) {
            if (userInputText.startsWith("/start")) {
                returnMessage = sendMessage.setText("Привет! Меня зовут ПростБот. Я веду учёт твоих расходов.\n" +
                ...
                returnMessage = sendMessage.setText("\nДоступные команды в данный момент:\n" +
                        "/help - список доступных команд\n" +
                        "/menu - вывод меню\n" +
                        "/table - ссылка на Google-таблицу.");
                //execute(sendMessage);
            } else if (userInputText.equalsIgnoreCase("/table")) {
                returnMessage = sendMessage.setText("https://docs.google.com/spreadsheets/d/1Y5jeX_ox55RgjUbQyDfaNJ8QFJEfjsIN5waS-kdYR0I/edit#gid=0");
                ...
                for (CategoryTitle t : CategoryTitle.values()) {
                    if (userInputText.contains(t.getTitle()) || userInputText.contains(t.getTitle().toLowerCase())) {
                        ...
                        returnString =  t.getTitle();
                        flag = false;
                    } else {
                        returnMessage = sendMessage.setText("Что-то не так. Попробуйте снова.");
